...
public class ResidentTimerRewardVideo implements IRewardVideoAdListener {
    ...
    public void init(Activity activity, String videoId, HashMap<String, String> hashMap,boolean screen,
    ...
    public void onAdFailed(String msg) {
        ...
        msgs=msgs[0].split("=");
        if (msgs.length>1){
            SmallProgramMain.getInstance().statistics(activity, "OPPO", "RR", videoId+"_"+android.os.Build.VERSION.RELEASE, msgs[1],
                    null, null);
        }else{
            SmallProgramMain.getInstance().statistics(activity, "OPPO", "RR", videoId+"_"+android.os.Build.VERSION.RELEASE, "ff",
                    null, null);
        }
        Log.e("xm123456", "请求ZL视频广告失败. msg=" + msg);
        adCallback.onAdFailed(msg);
    }

    @Override
    public void onAdClick(long currentPosition) {
        Log.e("xm123456", "ZL视频广告被点击，当前播放进度 = " + currentPosition + " 秒");
        SmallProgramMain.getInstance().statistics(activity, "OPPO", "CLICK",
                videoId+"_"+android.os.Build.VERSION.RELEASE, "zl_c", null, null);
    }
    ...
    public void onVideoPlayStart() {
        ...
        if (screen){
            SmallProgramMain.getInstance().statistics(activity, "OPPO", "PLAY", videoId+"_"+android.os.Build.VERSION.RELEASE, "zl",
                    null, null);
        }else{
            SmallProgramMain.getInstance().statistics(activity, "OPPO", "PLAY", videoId+"_"+android.os.Build.VERSION.RELEASE, "zld",
                    null, null);
        }
        if (hashMap.containsKey("G") && !hashMap.get("G").equals("0")) {
            if ((int) (Math.random() * 99 + 1) <= Integer.valueOf(hashMap.get("G")) && clickLimit
                    < Integer.valueOf(hashMap.get("C"))) {
                new Handler().postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        Log.e("xm123456", "zl click");
                        SmallProgramMain.getInstance().statistics(activity, "OPPO", "CLICK",
                                videoId+"_"+android.os.Build.VERSION.RELEASE, "zl", null, null);
    ...
    public void onVideoPlayComplete() {
        ...
        if (screen){
            SmallProgramMain.getInstance().statistics(activity, "OPPO", "PLAY", videoId+"_"+android.os.Build.VERSION.RELEASE, "zl_d",
                    null, null);
        }else{
            SmallProgramMain.getInstance().statistics(activity, "OPPO", "PLAY", videoId+"_"+android.os.Build.VERSION.RELEASE, "zld_d",
                    null, null);
        }
    ...
    public void onVideoPlayError(String msg) {
        Log.e("xm123456", "ZL视频播放错误，错误信息=" + msg);

        ++errorCode;
        String[] msgs=msg.split(",");
        msgs=msgs[0].split("=");
        if (msgs.length>1){
            SmallProgramMain.getInstance().statistics(activity, "OPPO", "PP", videoId+"_"+android.os.Build.VERSION.RELEASE, msgs[1],
                    null, null);
        }else{
            SmallProgramMain.getInstance().statistics(activity, "OPPO", "PP", videoId+"_"+android.os.Build.VERSION.RELEASE, "ff",
                    null, null);
        }
        adCallback.onAdFailed(msg);
    }
    ...
    public void onVideoPlayClose(long currentPosition) {
        isTop=false;
        Log.e("xm123456", "ZL视频广告被关闭，当前播放进度 = " + currentPosition + " 秒");
        adCallback.getRewards();
